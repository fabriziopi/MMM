...
find_best_pars_cv = function (start_day, train.length.percentage, vars, df, max_iter = 150) 
  
{
  ...
  if(!STEP) {
    ...
    max.rsquared=function(df, kpi, para, vars.lm, length.base, full_df) {
      ...
      # 
      # fit.cv <<- lm(as.formula(paste0('kpi~',vars.names.lm)), data = trans_df)
      
      cv <<- cv.lm(train.df, fit, m = 10)
      ...
      # # 
      # cv <- model$results$RMSE
      # 
      # return( 10*test_mape+ 1*train_mape + cv + neg.coef*100000+neg.base_tot*3 -0.0*coef(summary(fit))[13,3] -.0*coef(summary(fit))[14,3] +1*abs((cpr_tv-30)/30) + 0.*(abs((cpr_usem -20)/20))^2 )
      return(attributes(cv)$ms)
    }
    ...
    if (GLS == T){
      ...
      fit.da=DEoptim(max.rsquared, lower=rep(0.1, n_pars), upper=rep(0.95,n_pars),
                     ...
                     DEoptim.control(itermax=max_iter,NP=n_pars*10, parallelType = 1, parVar = list('train.df','vars','vars.lm', 'length.base', 'kpi',
                                                                                                    ...
                                                                                                    'trainControl', 'train', 'vars.names', 'mape', 'cv.lm'))
      )
